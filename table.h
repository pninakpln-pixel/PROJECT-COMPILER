...
/*
 * Searches for a symbol by name in the symbol table linked list.
 * Inputs:
 * - head - pointer to symbol table head.
 * - name - symbol name string to search for.
 * Output: Pointer to matching Symbol node if found, NULL otherwise.
 */
Symbol *find_symbol(Symbol *head, char *name);

/*
 * Updates the address of all data symbols by adding final IC value.
 * Inputs:
 * - head - pointer to symbol table head.
 * - ic - final instruction counter value (ICF) to add.
 * Output: SUCCESS_F on completion.
 */
int update_data_symbols_address(Symbol *head, int ic);

/*
 * Validates and adds a new symbol node to the symbol table linked list.
 * Inputs:
 * - head - double pointer to symbol table head pointer.
 * - name - symbol name string.
 * - address - memory address of the symbol.
 * - type - category type of the symbol (code, data, extern).
 * - line_number - current line number in source file.
 * - file_name - name of the processed file.
 * Output: SUCCESS_F (1) on success, ERROR_F (0) on syntax or duplicate error, MEMORY_ERROR (-1) on allocation failure.
 */
int add_symbol(Symbol **head, char *name, int address, SymbolType type,int line_number, char *file_name);

/*
 * Frees all allocated memory nodes in the symbol table linked list.
 * Inputs:
 * - head - pointer to symbol table head pointer.
 * Output: SUCCESS_F on completion.
 */
